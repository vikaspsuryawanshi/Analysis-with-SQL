...
SELECT DISTINCT Bands.name   #### the names of all Bands from the Songs table
FROM Bands
JOIN Albums ON Bands.id = Albums.band_id
JOIN Songs ON Albums.id = Songs.album_id;

SELECT *                   ####  Select the oldest album:
FROM Albums
ORDER BY release_year
LIMIT 1;

SELECT DISTINCT Bands.* FROM Bands  #### Get all the bands that have albums
JOIN Albums ON Bands.id = Albums.band_id;

SELECT Bands.* FROM Bands      #### Get all the bands that have albums
LEFT JOIN Albums ON Bands.id = Albums.band_id
WHERE Albums.id IS NULL;

SELECT * FROM Albums          ##### Get the longest album:
...
INSERT INTO Albums (name, release_year, band_id) VALUES ('Tribe of Force', 2010, @last_band_id);

### Delete the Band and Album you added in the previous question 
DELETE FROM Bands WHERE name = ('Seventh Wonder');    ############# Cannot delete or update a parent row: a foreign key constraint fails
...
SELECT Albums.name AS album_name, MAX(Songs.name) AS longest_song
FROM Albums JOIN Songs ON Albums.id = Songs.album_id
GROUP BY Albums.name
ORDER BY Albums.name;

##### Get the number of songs for each band:
SELECT Bands.name, COUNT(Songs.id) AS song_count
FROM Bands
LEFT JOIN Albums ON Bands.id = Albums.band_id
LEFT JOIN Songs ON Albums.id = Songs.album_id
GROUP BY Bands.id;

#### Create a decade column by dividing the year // 10 * 10:
SELECT *, release_year - release_year % 10 AS decade
FROM Albums;
...
### Find the album released between 2008 and 2013:
SELECT *
FROM Albums
